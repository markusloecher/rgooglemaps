`addPolysToMapTiles` <-structure(function#overlays polygons on map constructed from tiles
###This function plots/overlays polygons on a map tile composition. Typically, the polygons originate from a shapefile.
(
...
### or of class \link[sp]{SpatialPolygons} from the package sp
  ...
  #X=mt$X;Y=mt$Y
  rX = range(mt$X);rY = range(mt$Y);
  XY=LatLon2XY(lat= polys[,"Y"],lon= polys[,"X"],mt$zoom)
  ...
  polys.XY[,"X"] <- XY2[,"X"] #Rcoords$newX;
  polys.XY[,"Y"] <- XY2[,"Y2"]# Rcoords$newY;
